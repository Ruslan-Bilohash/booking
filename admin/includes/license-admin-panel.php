<?php
if (!function_exists('bk_admin_is_owner') || !bk_admin_is_owner()) {
    return;
}
...
$lp = $ta['license_page'] ?? [];
$pn = $lp['panel'] ?? [];
$licenseStatus = bk_license_status();
$licenseInfo = isset($licenseInfo) && is_array($licenseInfo)
    ? $licenseInfo
    : bk_license_verify_current(true);
...
<div class="adm-card adm-license-strip" id="shLicensePanel"
     data-api="<?= htmlspecialchars(bk_admin_url('api/license-status.php')) ?>"
    ...
    <div class="adm-card-body padded">
        <div class="adm-license-grid">
            ...
            <div class="adm-license-stat">
                ...
                <span class="adm-badge adm-badge-warn adm-license-renew-badge" id="shLicenseRenewSoon"<?= empty($licenseInfo['renew_soon']) ? ' style="display:none"' : '' ?>><?= htmlspecialchars($pn['renew_soon'] ?? 'Renew soon') ?></span>
            </div>
            <div class="adm-license-stat">
        ...
        <div class="adm-license-actions">
            ...
            <a href="<?= htmlspecialchars(bk_subscription_url()) ?>" class="adm-btn adm-btn-outline adm-btn-sm" <?= bk_subscription_external_attrs() ?>>
                <i class="fas fa-rotate"></i> <?= htmlspecialchars($pn['renew'] ?? 'Renew license') ?>
            </a>
            <a href="<?= htmlspecialchars(bk_admin_url('license.php')) ?>" class="adm-btn adm-btn-outline adm-btn-sm">
